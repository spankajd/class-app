...
const ColorPicker = ({ onChange }) => {
    ...
    // const selectedhexagon = useRef();
    // const divpreview = useRef();
    const { t, i18n } = useTranslation();

    const [entercolor, setEntercolor] = useState('#ff0000');
    const [html5colorpicker, setHtml5colorpicker] = useState('#ff0000');
    ...
    const mouseOverColor = (hex) => {
        console.log('mouseOverColor ', hex);
        // divpreview.current.style.visibility = "visible";
        // divpreview.current.style.backgroundColor = hex;
        document.body.style.cursor = "pointer";
    }
    const mouseOutMap = () => {
        if (hh == 0) {
            // divpreview.current.style.visibility = "hidden";
        } else {
            hh = 0;
        }
    ...
    const clickColor = (hex, seltop, selleft, html5) => {
        ...
        if ((seltop + 200) > -1 && selleft > -1) {
            // selectedhexagon.current.style.top = seltop + "px";
            // selectedhexagon.current.style.left = selleft + "px";
            // selectedhexagon.current.style.visibility = "visible";
        } else {
            // divpreview.current.style.backgroundColor = cObj.toHexString();
            // selectedhexagon.current.style.visibility = "hidden";
        }
        if(onChange) onChange(cObj.toHexString());
        setHtml5colorpicker(cObj.toHexString());
    }
    ...
    return (
        <div className={style.colorPicker}>
            <div className={style.title}>{t("background.color")}</div>
            <div className={style.pickColorWrapper}>
                <div className={style.subtitle}>{t("background.pickcolors")}</div>
